...
export function EstudiosContent() {
  ...
  useEffect(() => {
    const fetchEstudios = async () => {
      try {
        ...
        const pendientes = response.data.data.filter(
          (estudio) => estudio.estado === "Pendiente"
        );
  ...
  const handleDescargarTodos = () => {
    ...
    estudios.disponibles.forEach((estudio, idx) => {
      ...
      doc.setFontSize(11);
      doc.text(
        `Fecha: ${new Date(estudio.fecha_resultado).toLocaleDateString()}`,
        25,
        y
      );
      y += 6;
      doc.text(
        `Médico: Dr. ${estudio.nombre_medico} ${estudio.apellido_medico}`,
        25,
        y
      );
      y += 6;
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          Solicitado por Dr. {estudio.nombre_medico}{" "}
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-yellow-700 mt-2 font-medium">
                        {estudio.estado === "Pendiente"
                          ? "Pendiente de realización - Coordinar turno"
                          : "En proceso"}
                      </p>
                    </div>
                    {estudio.estado === "Pendiente" && (
